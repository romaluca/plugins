...
class ImageResizer {
  ...
  String resizeImageIfNeeded(
    ...
    try {
      ...
      File file = resizedImage(bmp, maxWidth, maxHeight, imageQuality, imageName, targetPath);
      if (targetPath == null)
  ...
  private File resizedImage(
      Bitmap bmp, Double maxWidth, Double maxHeight, Integer imageQuality, String outputImageName, String targetPath)
      throws IOException {
  ...
  private File createImageOnExternalDirectory(String name, Bitmap bitmap, int imageQuality, String targetPath)
    ...
    if (targetPath != null)
    {
      ...
      imageFile = createFile(externalFilesDirectory, name);
